...
@app.route('/number_of_instances')
def get_numbers():
    """Gives the total number of instances in the data

    Usage example : <hostname>/number_of_instances

    Returns
    -------
    number_of_instances : list of integers
        The list of user's ids
    """
    df = pd.read_csv(DATA_PATH)
    return jsonify(number_of_instances=len(df))
...
def get_exercises(user_id, learning_obj_id):
    ...
    Usage example : <hostname>/scores/user_id=231412&learning_obj_id=554363
